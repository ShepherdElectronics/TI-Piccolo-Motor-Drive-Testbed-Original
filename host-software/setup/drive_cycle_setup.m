%% ================================================================
%  DRIVE-CYCLE COMMAND AND HOST-MODEL SETUP
%  Compressed FTP-75 profile with smoothed speed and load commands
%
%  Host model:
%    dual_motor_drive_cycle_host.slx (preferred)
%    Legacy model names are accepted through the candidate list below.
%
%  Creates:
%    FTP75_motorRPM_sig
%    FTP75_IqRef_A_sig
...
%    - Ramps RPM and Iq to zero at the end.
%    - Holds zero after the trial to prevent extrapolation.
%    - Sets Simulink StopTime to final command time.
%    - Assumes Manual Switch blocks were deleted.
%% ================================================================
...
% Kill commands at end
killAtEnd     = true;
endRamp_s     = 2.0;      % smooth ramp to zero before final stop
endKillHold_s = 5.0;      % zero hold after ramp
...
if miniFtpModel
    ...
    fprintf('  Expected run before end-kill hold = %.1f s\n\n', t(end) + startDelay_s);
end
...
%% ================================================================
%  4. MOTOR 2 LOAD-CURRENT COMMAND
%% ================================================================
...
if startDelay_s > 0
    ...
    fprintf('  Trial duration now   = %.2f s\n\n', t_iq(end));
end

%% ================================================================
%  4D. KILL MOTOR COMMANDS AT END
%      Ramps RPM and Iq to zero, holds zero, and prevents extrapolation.
%% ================================================================

if killAtEnd
    dt_cmd = median(diff(t_iq));

    nRamp = max(3, round(endRamp_s / dt_cmd));
    nRamp = min(nRamp, numel(t_iq));

    rampGain = linspace(1, 0, nRamp)';
    idxRamp = (numel(t_iq)-nRamp+1):numel(t_iq);

    motor_rpm_cmd(idxRamp) = motor_rpm_cmd(idxRamp) .* rampGain;
    Iq_cmd_A(idxRamp)      = Iq_cmd_A(idxRamp)      .* rampGain;

    motor_rpm_raw(idxRamp)           = motor_rpm_raw(idxRamp)           .* rampGain;
    motor_rpm_raw_enveloped(idxRamp) = motor_rpm_raw_enveloped(idxRamp) .* rampGain;
    motor_rpm_smooth(idxRamp)        = motor_rpm_smooth(idxRamp)        .* rampGain;
    motor_rpm_slew(idxRamp)          = motor_rpm_slew(idxRamp)          .* rampGain;

    Iq_raw_A(idxRamp)    = Iq_raw_A(idxRamp)    .* rampGain;
    Iq_smooth_A(idxRamp) = Iq_smooth_A(idxRamp) .* rampGain;
    Iq_slew_A(idxRamp)   = Iq_slew_A(idxRamp)   .* rampGain;

    v_kmh(idxRamp)    = v_kmh(idxRamp)    .* rampGain;
    v_mps(idxRamp)    = v_mps(idxRamp)    .* rampGain;
    v_iq(idxRamp)     = v_iq(idxRamp)     .* rampGain;
    v_smooth(idxRamp) = v_smooth(idxRamp) .* rampGain;

    accel_mps2(idxRamp) = accel_mps2(idxRamp) .* rampGain;

    F_roadload_N(idxRamp) = F_roadload_N(idxRamp) .* rampGain;
    F_inertia_N(idxRamp)  = F_inertia_N(idxRamp)  .* rampGain;
    F_total_N(idxRamp)    = F_total_N(idxRamp)    .* rampGain;

    T_roadload_motor_Nm(idxRamp) = T_roadload_motor_Nm(idxRamp) .* rampGain;
    T_inertia_motor_Nm(idxRamp)  = T_inertia_motor_Nm(idxRamp)  .* rampGain;
    T_total_motor_Nm(idxRamp)    = T_total_motor_Nm(idxRamp)    .* rampGain;

    motor_rpm_raw(end)           = 0;
    motor_rpm_raw_enveloped(end) = 0;
    motor_rpm_smooth(end)        = 0;
    motor_rpm_slew(end)          = 0;
    motor_rpm_cmd(end)           = 0;
    ...
    T_roadload_motor_Nm(end) = 0;
    T_inertia_motor_Nm(end)  = 0;
    T_total_motor_Nm(end)    = 0;

    t_end_hold = (t_iq(end) + dt_cmd : dt_cmd : t_iq(end) + endKillHold_s)';
    nEnd = numel(t_end_hold);

    t_iq = [t_iq; t_end_hold];
    t   = t_iq;

    motor_rpm_raw           = [motor_rpm_raw;           zeros(nEnd,1)];
    motor_rpm_raw_enveloped = [motor_rpm_raw_enveloped; zeros(nEnd,1)];
    motor_rpm_smooth        = [motor_rpm_smooth;        zeros(nEnd,1)];
    motor_rpm_slew          = [motor_rpm_slew;          zeros(nEnd,1)];
    motor_rpm_cmd           = [motor_rpm_cmd;           zeros(nEnd,1)];

    Iq_raw_A    = [Iq_raw_A;    zeros(nEnd,1)];
    Iq_smooth_A = [Iq_smooth_A; zeros(nEnd,1)];
    Iq_slew_A   = [Iq_slew_A;   zeros(nEnd,1)];
    Iq_cmd_A    = [Iq_cmd_A;    zeros(nEnd,1)];

    v_kmh    = [v_kmh;    zeros(nEnd,1)];
    v_mps    = [v_mps;    zeros(nEnd,1)];
    v_iq     = [v_iq;     zeros(nEnd,1)];
    v_smooth = [v_smooth; zeros(nEnd,1)];

    accel_mps2 = [accel_mps2; zeros(nEnd,1)];

    F_roadload_N = [F_roadload_N; zeros(nEnd,1)];
    F_inertia_N  = [F_inertia_N;  zeros(nEnd,1)];
    F_total_N    = [F_total_N;    zeros(nEnd,1)];

    T_roadload_motor_Nm = [T_roadload_motor_Nm; zeros(nEnd,1)];
    T_inertia_motor_Nm  = [T_inertia_motor_Nm;  zeros(nEnd,1)];
    T_total_motor_Nm    = [T_total_motor_Nm;    zeros(nEnd,1)];

    fprintf('\nKill-at-end added:\n');
    fprintf('  End ramp duration    = %.2f s\n', endRamp_s);
    fprintf('  End zero hold        = %.2f s\n', endKillHold_s);
    fprintf('  Final signal time    = %.2f s\n\n', t_iq(end));
end
...
%% ================================================================
%  4E. FINAL SIMULINK FROM WORKSPACE SIGNALS
%% ================================================================
...
%% ================================================================
%  5. PLOT DRIVE-CYCLE COMMANDS
%% ================================================================

figure('Name','Drive-Cycle Test Mini FTP-75 Rounded RPM and Doubled Load Command','Color','w');
tiledlayout(8,1);
...
%% ================================================================
%  6. OPEN HOST MODEL AND SET DRIVE-CYCLE FROM-WORKSPACE BLOCKS
%     Manual switches were deleted, so no switch tracing is needed.
%% ================================================================

preferredModelNames = { ...
    'dual_motor_drive_cycle_host', ...
    'drive_cycle_host_model', ...
    'pmsm_dyno_drive_cycle_host' ...
};
...
searchRoots = { ...
    scriptDir, ...
    projectRoot, ...
    fullfile(projectRoot, 'host-software', 'supervisory-models'), ...
    fullfile(projectRoot, 'embedded-control', 'target-models'), ...
    pwd ...
};

for iName = 1:numel(preferredModelNames)
    for iRoot = 1:numel(searchRoots)
        candidateModelFiles{end+1,1} = fullfile( ...
            searchRoots{iRoot}, [preferredModelNames{iName} '.slx']); %#ok<SAGROW>
    end
end

% Add neutral filename matches discovered under the project root.
recursiveModels = dir(fullfile(projectRoot, '**', '*.slx'));
for iModel = 1:numel(recursiveModels)
    modelNameLower = lower(recursiveModels(iModel).name);
    if contains(modelNameLower, 'drive') && ...
       contains(modelNameLower, 'cycle') && ...
       contains(modelNameLower, 'host')
        candidateModelFiles{end+1,1} = fullfile( ...
            recursiveModels(iModel).folder, recursiveModels(iModel).name); %#ok<SAGROW>
    end
end
...
if isempty(modelFile)
    error(['Could not find a supported drive-cycle host model. Checked: ' ...
        strjoin(candidateModelFiles, ', ')]);
end

fprintf('\nOpening drive-cycle host model:\n  %s\n', modelFile);
...
set_param(loadedModelName, 'StopTime', num2str(t_iq(end)));
fprintf('  Model StopTime set to %.2f s\n', t_iq(end));
...
if isempty(rpmBlock)
    for k = 1:numel(fromWsBlocks)
        ...
        blkName = lower(string(get_param(blk, 'Name')));
        blkPath = lower(string(blk));

        if contains(blkName, "rpm") || ...
           contains(blkName, "speed") || ...
           contains(blkPath, "rpm") || ...
           contains(blkPath, "speed")
            rpmBlock = blk;
            break;
        end
    end
...
if isempty(iqBlock)
    for k = 1:numel(fromWsBlocks)
        ...
        if contains(blkName, "iq") || ...
           contains(blkName, "iqref") || ...
           contains(blkName, "load") || ...
           contains(blkPath, "iq") || ...
           contains(blkPath, "iqref") || ...
           contains(blkPath, "load")
            iqBlock = blk;
            break;
        end
    end
...
if isempty(rpmBlock)
    warning(['Could not automatically find the RPM From Workspace block. ' ...
             'Manually set its VariableName to FTP75_motorRPM_sig.']);
else
...
if isempty(iqBlock)
    warning(['Could not automatically find the Iq From Workspace block. ' ...
             'Manually set its VariableName to FTP75_IqRef_A_sig.']);
else
...
fprintf('\nDrive-cycle host model configured.\n');
fprintf('  Model opened: %s\n', loadedModelName);
fprintf('  RPM From Workspace variable should be FTP75_motorRPM_sig.\n');
fprintf('  Iq From Workspace variable should be FTP75_IqRef_A_sig.\n');
fprintf('  StopTime should match final command time.\n');
fprintf('  Manual Switch logic skipped because switches were deleted.\n\n');
